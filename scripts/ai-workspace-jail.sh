#!/bin/sh
# ai-workspace-jail.sh — used as ClaudeAgentOptions.cli_path for CONFINED sessions.
...
# jail — M-9) and a SANITIZED settings.json (AI_WORKSPACE_JAIL_SETTINGS_FILE, provided
# by the bridge with MCP-approval keys stripped — M-7). In BYO-key mode
# (AI_WORKSPACE_JAIL_NO_OWNER_CREDS=1, set by the bridge when a per-user
...
set -eu
: "${AI_WORKSPACE_PROJECT_ROOT:?jail: AI_WORKSPACE_PROJECT_ROOT unset}"
: "${AI_WORKSPACE_REAL_CLAUDE:?jail: AI_WORKSPACE_REAL_CLAUDE unset}"
: "${HOME:?jail: HOME unset}"
CREDS_DIR="${AI_WORKSPACE_CLAUDE_CREDS_DIR:-$HOME/.claude}"
...
#   bwrap <mounts> [<owner-creds>] <namespaces> "$AI_WORKSPACE_REAL_CLAUDE" <claude args>

# 4) the real claude binary, immediately before its own args.
set -- "$AI_WORKSPACE_REAL_CLAUDE" "$@"

# 3) namespaces / env / working dir.
set -- \
  --setenv HOME "$HOME" --chdir "$AI_WORKSPACE_PROJECT_ROOT" \
...
#    (AI_WORKSPACE_JAIL_NO_OWNER_CREDS=1) these are skipped, so ~/.claude stays an empty
#    tmpfs and the owner's .credentials.json is ABSENT inside the jail.
if [ "${AI_WORKSPACE_JAIL_NO_OWNER_CREDS:-}" != "1" ]; then
  ...
  set -- \
    ...
    "$@"
  # M-7: mount the bridge-SANITIZED settings.json (AI_WORKSPACE_JAIL_SETTINGS_FILE), NOT
  ...
  # mount nothing.
  if [ -n "${AI_WORKSPACE_JAIL_SETTINGS_FILE:-}" ]; then
    set -- \
      --ro-bind-try "$AI_WORKSPACE_JAIL_SETTINGS_FILE" "$HOME/.claude/settings.json" \
      "$@"
  fi
fi
...
set -- \
  ...
  --bind "$AI_WORKSPACE_PROJECT_ROOT" "$AI_WORKSPACE_PROJECT_ROOT" \
  --tmpfs "$HOME/.claude" \
  "$@"
