...
наличии ~/.claude). Плюс выставляется AI_WORKSPACE_JAIL_NO_OWNER_CREDS=1 — сигнал
ai-workspace-jail.sh не бинд-маунтить owner-креды (сессия платит своим ключом).
"""
import inspect
...
def test_spawn_env_sets_no_owner_creds_signal_when_key_provided() -> None:
    ...
    env = _bridge()._spawn_env(anthropic_api_key="sk-ant-user-123")
    assert env["AI_WORKSPACE_JAIL_NO_OWNER_CREDS"] == "1"
...
def test_spawn_env_no_injection_without_key(monkeypatch) -> None:
    ...
    monkeypatch.delenv("AI_WORKSPACE_JAIL_NO_OWNER_CREDS", raising=False)
    monkeypatch.setattr(
        ClaudeBridge, "_subscription_creds_present", staticmethod(lambda: True)
    )
    ...
    assert "AI_WORKSPACE_JAIL_NO_OWNER_CREDS" not in env
...
def test_build_sdk_options_threads_api_key_into_env() -> None:
    ...
    assert opt.kwargs["env"]["AI_WORKSPACE_JAIL_NO_OWNER_CREDS"] == "1"
